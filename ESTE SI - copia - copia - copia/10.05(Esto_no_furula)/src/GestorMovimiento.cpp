	...
	if (tablero.ocupado[int(pos1.x)][int(pos1.y)] == 4){
		...
		//	//y quiero ir una casilla que está en diagonal o recto desde mi posición
			if (abs(pos2.x - pos1.x) == abs(pos2.y - pos1.y)) {//DIAGONAL
				...
				printf("Jugador pos1 ACTUALIZANDOOOOOOO % d     ", tablero.jugador[int(pos1.x)][int(pos1.y)]);
				printf("Ocupado pos1 ACTUALIZANDOOOOOOO %d\n", tablero.ocupado[int(pos1.x)][int(pos1.y)]);
				printf("Jugador pos2 hhhhh% d    ", tablero.jugador[int(pos2.x)][int(pos2.y)]);
				printf("Ocupado pos2 hhhhhhh%d\n", tablero.ocupado[int(pos2.x)][int(pos2.y)]);
				retorno.x = 1;
				...
				printf("Jugador pos1 ACTUALIZANDOOOOOOO % d     ", tablero.jugador[int(pos1.x)][int(pos1.y)]);
				printf("Ocupado pos1 ACTUALIZANDOOOOOOO %d\n", tablero.ocupado[int(pos1.x)][int(pos1.y)]);
				printf("Jugador pos2  hhhh% d    ", tablero.jugador[int(pos2.x)][int(pos2.y)]);
				printf("Ocupado pos2 hhhhhh%d\n", tablero.ocupado[int(pos2.x)][int(pos2.y)]);
				retorno.x = 1;
			}
		}
		...
		//SI LA CASILLA ESTÁ OCUPADA
			...
			if (tablero.jugador[int(pos2.x)][int(pos2.y)] == tablero.jugador[int(pos1.x)][int(pos1.y)])//me quiero comer a uno de mi equipo.
				...
				printf("Jugador pos1 ACTUALIZANDOOOOOOO % d     ", tablero.jugador[int(pos1.x)][int(pos1.y)]);
				printf("Ocupado pos1 ACTUALIZANDOOOOOOO %d\n", tablero.ocupado[int(pos1.x)][int(pos1.y)]);
				printf("Jugador pos2 % d    ", tablero.jugador[int(pos2.x)][int(pos2.y)]);
				printf("Ocupado pos2 %d\n", tablero.ocupado[int(pos2.x)][int(pos2.y)]);
				retorno.y = 1;
				retorno.x = 1;
			}
