...
Vector2D Reina::mov_posible(Vector2D v,int n)
{
	...
	if (n % 2 == 0) { //SEGUNDO CLICK
		...
		if (tablero.getOcupado(int(pos1.y), int(pos1.x)) == REINA) {
			...
			if (tablero.getOcupado(int(pos2.y), int(pos2.x))== 0) { //si la casilla no está ocupada
			//	//y quiero ir una casilla que está en diagonal o recto desde mi posición
				if (abs(pos2.y - pos1.y) == abs(pos2.x - pos1.x)) {//DIAGONAL
					...
					columna = int (pos2.x);
					fila = int (pos2.y);
					tablero.setOcupado(int(pos1.y), int(pos1.x),0);
					tablero.setOcupado(int(pos2.y), int(pos2.x),4);

					tablero.jugador[int(pos1.y)][int(pos1.x)] = 2;
			...
			else if (tablero.getOcupado(int(pos2.y), int(pos2.x)) != 0) {
