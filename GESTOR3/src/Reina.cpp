...
Vector2D Reina::mov_posible(Vector2D pos1,Vector2D pos2,int n)
{
		...
		if (tablero.ocupado[int(pos1.y)][int(pos1.x)] == REINA) {
			...
			//	//y quiero ir una casilla que está en diagonal o recto desde mi posición
				if (abs(pos2.y - pos1.y) == abs(pos2.x - pos1.x)) {//DIAGONAL
					...
					printf("Jugador pos1 ACTUALIZANDOOOOOOO % d     ", tablero.jugador[int(pos1.y)][int(pos1.x)]);
					printf("Ocupado pos1 ACTUALIZANDOOOOOOO %d\n", tablero.ocupado[int(pos1.y)][int(pos1.x)]);
					printf("Jugador pos2 hhhhh% d    ", tablero.jugador[int(pos2.y)][int(pos2.x)]);
					...
					printf("Jugador pos1 ACTUALIZANDOOOOOOO % d     ", tablero.jugador[int(pos1.y)][int(pos1.x)]);
					printf("Ocupado pos1 ACTUALIZANDOOOOOOO %d\n", tablero.ocupado[int(pos1.y)][int(pos1.x)]);
					printf("Jugador pos2  hhhh% d    ", tablero.jugador[int(pos2.y)][int(pos2.x)]);
